...
# --- OUTPUT FILES (Specific to sentiment) ---
EMBEDDINGS_OUTPUT_PATH = 'embeddings_sentiment.npy'
LABELS_OUTPUT_PATH = 'labels_sentiment.csv'
...
def main():
    """
    Main function to load data, generate embeddings for sentiment analysis, and save files.
    """
    # 1. Setup device
    ...
    # 3. Load and Prepare Data (Focus on sentiment)
    print(f"Loading data from {INPUT_CSV_PATH}...")
    df = pd.read_csv(INPUT_CSV_PATH)

    # Drop rows where 'text' or 'sentiment' columns have missing values
    df = df.dropna(subset=['text', 'sentiment']).copy()

    # Clean the text and sentiment columns
    df['text'] = df['text'].astype(str).str.strip()
    df['sentiment'] = df['sentiment'].astype(
        str).str.strip().str.lower()

    # Filter out any rows where the sentiment label is 'na'
    df = df[df['sentiment'] != 'na'].reset_index(drop=True)

    texts = df['text'].tolist()
    labels = df[['sentiment']]

    print(f"Found {len(texts)} valid text samples for sentiment analysis.")
    ...
    print(f"Sentiment labels saved to: {LABELS_OUTPUT_PATH}")
