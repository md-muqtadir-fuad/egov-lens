...
# Drop rows where 'text' or 'responsiveness' are missing
df = df.dropna(subset=['text', 'responsiveness']).copy()

# Clean the 'text' and 'responsiveness' columns
df['text'] = df['text'].astype(str).str.strip()
df['responsiveness'] = df['responsiveness'].astype(str).str.strip().str.lower()

# IMPORTANT: Filter out rows where 'responsiveness' is 'na'
# We only want to train on meaningful labels.
...
# ========== 2. Split ==========
X_train, X_temp, y_train, y_temp = train_test_split(df['text'], df['responsiveness'], test_size=0.3, stratify=df['responsiveness'], random_state=42)
